...
Bash script to install Debian 13, KDE Plasma 6, xserver-xorg with ZFS on Root and ZFSBootMenu, from : https://docs.zfsbootmenu.org/en/latest/guides/debian
...
- A live installation environment (e.g., Debian Live)
   1. **Remarque :**
      ```bash
      The first task is to download the latest debian-live-13.X.X-amd64-standard.iso, this necessary to don''t have a miss matches packages between the live iso image and the "apt update && apt upgrade" instruction, causing the system to not to be able loading the ZFS modules
      ```
